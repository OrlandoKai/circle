        ...
        for filter_key in task.instances[0].filtered_resps:
            ...
            for doc_id, doc in doc_iterator:
                ...
                if log_samples:
                    ...
                    rag_filename_target_pairs = []
                    for req in requests:
                        for resp in req.resps:
                            names = getattr(resp, "rag_sample_filenames", None)
                            if not isinstance(names, list):
                                continue
                            targets = getattr(resp, "rag_sample_targets", None)
                            if isinstance(targets, list) and len(targets) == len(names):
                                rag_filename_target_pairs.extend(
                                    [
                                        (str(name), str(target))
                    ...
                    if len(rag_filename_target_pairs) > 0:
                        rag_filename_target_pairs = list(dict.fromkeys(rag_filename_target_pairs))
                    ...
                    example = {
                        ...
                        "rag_random_sample_filenames": rag_random_sample_filenames,
                        "rag_random_sample_targets": rag_random_sample_targets,
                    }
