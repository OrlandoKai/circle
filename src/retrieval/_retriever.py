...
class Retriever:
    ...
    def __init__(
        ...
        self._last_retrieved_results: list[list[dict[str, Any]]] = []
    ...
    def get_last_retrieved_filenames(self) -> list[list[str]]:
        """Return source filenames for the latest retrieval call, grouped by query."""
        filenames_by_query: list[list[str]] = []
        for results in self._last_retrieved_results:
            names: list[str] = []
            for result in results:
                candidate = result.get("visual") or result.get("image_path")
                if not isinstance(candidate, str):
                    continue
                name = Path(candidate).name
                if len(name) > 0:
                    names.append(name)
            filenames_by_query.append(names)
        return filenames_by_query

    def get_last_retrieved_targets(self) -> list[list[str]]:
        """Return per-query true labels for the latest retrieval call."""
        targets_by_query: list[list[str]] = []
        for results in self._last_retrieved_results:
            targets: list[str] = []
            for result in results:
                # Prefer original semantic label in caption; fallback to target when needed.
                candidate = result.get("caption")
                if candidate is None:
                    candidate = result.get("target")
                targets.append(str(candidate) if candidate is not None else "")
            targets_by_query.append(targets)
    ...
    def retrieve_and_prepare(
        ...
        # Keep retrieval metadata for downstream logging (e.g., random sampled filenames).
        self._last_retrieved_results = [
        ...
        if grab_data_type == "image":
            ...
            for batch_idx, results in enumerate(results_set):
                ...
                for result in results:
                    ...
                    # Tensor databases usually store filesystem paths in `visual`.
                    if key is None:
                        visual = result.get("visual")
                        if isinstance(visual, str) and len(visual) > 0:
                            key = visual
    ...
    def get_images(self, keys: list[str], from_tar: bool = False) -> dict[str, Image.Image | None]:
        ...
        all_images: dict[str, Image.Image | None] = {}
        ...
        for key in keys:
            ...
            if fp.exists() and fp.is_file():
                try:
                    all_images[key] = Image.open(fp).convert("RGB")
                except Exception:
                    all_images[key] = None
            else:
                retrieval_keys.append(key)

        if len(retrieval_keys) > 0:
            all_images.update(get_images_by_key(retrieval_keys, root, from_tar=from_tar))
